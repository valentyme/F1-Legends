...
public class CircuitoSpa extends Circuito {
    ...
    private void trazarPitLane(GraphicsContext gc) {
        // Paralelo a la recta de meta (y=540), desplazado a y=550
        // Mismo sentido: de x=470 (entrada) a x=215 (salida)
        gc.beginPath();
        gc.moveTo(470, 550);
        gc.lineTo(215, 550);
        gc.stroke();
    ...
    private void dibujarPitLaneDetalle(GraphicsContext gc) {
        ...
        // Boxes (debajo del pit lane)
        gc.setFill(Color.rgb(72, 72, 80));
        gc.setStroke(Color.rgb(105, 105, 112));
        gc.setLineWidth(0.6);
        for (int i = 0; i < 14; i++) {
            double bx = 225 + i * 17.0;
            gc.fillRect(bx, 558, 15, 9);
            gc.strokeRect(bx, 558, 15, 9);
        }
    ...
    public Punto getPuntoEntradaPit() { return new Punto(470, 540); }
    public Punto getPuntoSalidaPit()  { return new Punto(215, 540); }

    /** t ∈ [0,1]: recorre el pit lane de entrada (der) a salida (izq) */
    public Punto calcularPosicionPitLane(double t) {
        if (t <= 0.15) {
            // Curva de entrada: baja de y=540 a y=550
            return calcularBezier(t / 0.15,
                    new Punto(470, 540), new Punto(474, 540),
                    new Punto(474, 550), new Punto(470, 550));
        } else if (t <= 0.85) {
            // Recta del pit lane de derecha a izquierda
            return interpolarLinea((t - 0.15) / 0.70,
                    new Punto(470, 550), new Punto(215, 550));
        } else {
            // Curva de salida: sube de y=550 a y=540
            return calcularBezier((t - 0.85) / 0.15,
                    new Punto(215, 550), new Punto(211, 550),
                    new Punto(211, 540), new Punto(215, 540));
        }
    }
